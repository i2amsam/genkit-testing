...
async function runNewEvaluation(manager, request) {
    ...
    const evalDataset = await runInference({
        manager,
        actionRef,
        evalFlowInput: types_1.EvalInferenceInputSchema.parse({ samples: dataset }),
...
async function bulkRunAction(params) {
    ...
    let testCases = Array.isArray(evalFlowInput)
        ...
        : evalFlowInput.samples.map((c) => ({
            ...
            testCaseId: c.testCaseId ?? (0, utils_1.generateTestCaseId)(),
        }));
    let states = [];
    let evalInputs = [];
...
async function runFlowAction(params) {
    ...
    let state;
    try {
        const flowInput = types_1.FlowActionInputSchema.parse({
            start: {
